...
parser.add_argument('--detbatch', type=int, default=1, #5,
                    help='detection batch size PER GPU')
parser.add_argument('--posebatch', type=int, default=1, #64,
...
if __name__ == "__main__":
    ...
    #mode, input_source = check_input()
    #mode = 'video'
    input_video_path = "testVideos/"
    #input_video_path = "/home/yin/Videos/"
    fileList = os.listdir(input_video_path)
 #   output_result_path = "results/Videos/"
    output_result_path = "testResults/"
    
    videoFileList = []
    suffix = ".avi"
    for file in fileList:
        #str = file #file[len(file)-4:]
        #print(f"================= {str}\n")a_vibeSample.mp
    ...
    #"/gdrive/MyDrive/FencingVideoDB/20220304_140617000_iOS.mp4"]
    print(videoFileList)

    for video_file in videoFileList:
    #if True:
        mode = 'video'
        input_source = input_video_path + video_file
        output_dir = output_result_path + video_file.replace(suffix, '')
        #print(f'output_dir:{output_dir}')
        if os.path.exists(output_dir):
            print(f'{video_file} has already been processed.')
            continue
